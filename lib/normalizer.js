  ...
  function normalize(nd, context) {
    ...
    case 'AssignmentExpression':
      if(nd.operator === '=') {
        if(nd.left.type === 'Identifier') {
          ...
          if(nd.left.computed)
            ast.setAnnotation(lhs, 'isComputed', true);
        ...
        if(nd.left.type === 'Identifier') {
          var tmp = context.genTmp(), tmp2 = context.genTmp(), tmp3 = context.genTmp();
          // TODO: the first two assignments are for backwards compatibility with previous implementation; could be inlined
          return rec(rhs, tmp).concat(new ast.ExpressionStatement(new ast.AssignmentExpression('=', new ast.Identifier(tmp2), new ast.Identifier(nd.left.name))),
                                      new ast.ExpressionStatement(new ast.AssignmentExpression('=', new ast.Identifier(tmp3), new ast.Identifier(tmp))),
                                      new ast.ExpressionStatement(new ast.AssignmentExpression('=', new ast.Identifier(nd.left.name), 
                                                                                                    new ast.BinaryExpression(op, new ast.Identifier(tmp2), new ast.Identifier(tmp3)))));
        } else if(nd.left.type === 'MemberExpression') {
    ...
    case 'UnaryExpression': 
      ...
      if(op === 'delete') {
        ...
        var tmp = context.genTmp();
        return rec(nd.argument, new ast.Identifier(tmp))
    ...
    case 'ContinueStatement':
      if(nd.label)
        return [new ast.BreakStatement(new ast.Identifier(nd.label))];
