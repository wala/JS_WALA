...
define(function(require, exports) {
  ...
  function normalize(nd, options) {
    ...
    // the main normalizing function
    function process(nd, context) {

      // convenience function for recursively invoking 'process', possibly with a new target
      function rec(nd) {
        // if second argument is a number, rec is being mapped over a list of statements;
        // force use of a fresh target
        if(typeof arguments[1] === 'number')
          arguments[1] = null;
        
        // if second argument is a string, interpret it as the new target; if it is null,
        // use fresh target variable
        if(typeof arguments[1] === 'string' || arguments[1] === null) {
          context.pushTarget(arguments[1]);
          var res = process(nd, context);
          context.popTarget();
          return res;
        } else {
          return process(nd, context);
        }
      }
      ...
      // normalize function declaration/expression nd
      function normalizeFunction(nd) {
        context = context.enterFunction(nd);
        
        if(options.unify_ret) {
          context.ret_label = context.genTmp(true);
          context.ret_var = context.genTmp();
        }
        ...
        if(options.unify_ret)
          body = [new ast.LabeledStatement(new ast.Identifier(context.ret_label), new ast.BlockStatement(body)),
                  new ast.ReturnStatement(new ast.Identifier(context.ret_var))];
        ...
        var fundecls = context.scope.decls.flatmap(function(decl) {
        ...
        var local_names = [];
        context.scope.decls.forEach(function(decl) {
      ...
      case 'Identifier':
        ...
        if(context.isGlobal(nd.name)) {
          ...
          if(!options.reference_errors || context.scope.isDeclaredGlobal(nd.name)) {
            res[1] = new ast.ExpressionStatement(new ast.AssignmentExpression('=', new ast.Identifier(context.getTarget()), new ast.MemberExpression(new ast.Identifier('__global'), new ast.Identifier(tmp), true)));
          } else {
            // TODO: even if it isn't shadowed, some clown may have overwritten ReferenceError...
            if(!context.isGlobal('ReferenceError'))
             ...
             *   throw tmp5;
             * } */
            var tmp2 = context.genTmp(), tmp3 = context.genTmp(), tmp4 = context.genTmp(), tmp5 = context.genTmp();
          ...
          // locals are easy
          res = [new ast.ExpressionStatement(new ast.AssignmentExpression('=', new ast.Identifier(context.getTarget()), new ast.Identifier(nd.name)))];
        }
        
        // handle possible 'with' bindings
        var with_bindings = context.scope.possibleWithBindings(nd.name);
          ...
          with_bindings.forEach(function(with_var) {
            ...
            res = [new ast.ExpressionStatement(new ast.AssignmentExpression('=', new ast.Identifier(tmp2), new ast.BinaryExpression('in', new ast.Identifier(tmp), new ast.Identifier(with_var)))),
                   new ast.IfStatement(new ast.Identifier(tmp2),
                                       mkBlock([new ast.ExpressionStatement(new ast.AssignmentExpression('=', new ast.Identifier(context.getTarget()), new ast.MemberExpression(new ast.Identifier(with_var), new ast.Identifier(tmp), true)))]),
                                       mkBlock(res))];
          });
      ...
      // variable declarations are collected by normalizeFunction() above; all we need to do here is to extract initializers into assignments
      ...
      case 'ReturnStatement':
        if(options.unify_ret) {
          var res = nd.argument ? rec(nd.argument, context.ret_var) : [];
          res.push(new ast.BreakStatement(new ast.Identifier(context.ret_label)));
          return res;
        } else {
          if(nd.argument) {
            var tmp = context.genTmp();
      ...
      case 'AssignmentExpression':
        if(nd.operator === '=') {
          ...
          if(nd.left.type === 'Identifier') {
            ...
            var with_bindings = context.scope.possibleWithBindings(nd.left.name);
            if(context.isGlobal(nd.left.name)) {
              ...
              if(!context.isLocal(nd.left.name))
                ast.setAttribute(context.scope.lookup(nd.left.name), 'exposed', true);
              
              if(context.hasTarget() || with_bindings.length) {
                right = rec(nd.right);
                res = [new ast.ExpressionStatement(new ast.AssignmentExpression('=', new ast.Identifier(nd.left.name), new ast.Identifier(context.getTarget())))];
              } else {
                context.setTarget(nd.left.name);
                right = rec(nd.right, nd.left.name);
                res = [];
              }
            }
            ...
            if(with_bindings.length) {
              ...
              with_bindings.forEach(function(with_var) {
                ...
                res = [new ast.ExpressionStatement(new ast.AssignmentExpression('=', new ast.Identifier(tmp2), new ast.BinaryExpression('in', new ast.Identifier(tmp), new ast.Identifier(with_var)))),
                       new ast.IfStatement(new ast.Identifier(tmp2),
                                           mkBlock([new ast.ExpressionStatement(new ast.AssignmentExpression('=', new ast.MemberExpression(new ast.Identifier(with_var), new ast.Identifier(tmp), true), new ast.Identifier(context.getTarget())))]),
                                           mkBlock(res))];
              });
      ...
      case 'CallExpression':
        if(nd.callee.type === 'MemberExpression') {
          ...
          if(nd.callee.name === 'eval') {
            ...
            var tmps = context.genTmps(nd.arguments.length);
            var args = nd.arguments.flatmap(function(arg, i) { return rec(arg, tmps[i]); });
            var res = [new ast.ExpressionStatement(new ast.AssignmentExpression('=', new ast.Identifier(context.getTarget()),
                                                                                     new ast[nd.type](new ast.Identifier(tmp), tmps.map(function(tmp) { return new ast.Identifier(tmp); }))))];
            
            var with_bindings = context.scope.possibleWithBindings(nd.callee.name);
            if(with_bindings.length) {
              var name_tmp = context.genTmp();
              ...
              with_bindings.forEach(function(with_var) {
                ...
                res = [new ast.ExpressionStatement(new ast.AssignmentExpression('=', new ast.Identifier(tmp2), new ast.BinaryExpression('in', new ast.Identifier(name_tmp), new ast.Identifier(with_var)))),
                       new ast.IfStatement(new ast.Identifier(tmp2),
                                           mkBlock([new ast.ExpressionStatement(new ast.AssignmentExpression('=', new ast.Identifier(context.getTarget()),
                                                                                                                  new ast.CallExpression(new ast.MemberExpression(new ast.Identifier(with_var), new ast.Identifier(name_tmp), true), 
                                                                                                                                         tmps.map(function(tmp) { return new ast.Identifier(tmp); }))))]),
                                           mkBlock(res))];
              });
      ...
      case 'TryStatement':
        if(nd.handlers.length > 0 && nd.finalizer) {
          ...
          context = context.leaveCatchClause();
      ...
      case 'WithStatement':
        var tmp = context.genTmp();
        var preamble = rec(nd.object, tmp);
        var new_ctxt = context.enterWith(tmp);
        var body = rec(nd.body, new_ctxt);
        context.leaveWith();
    ...
    return process(nd, context.makeRootContext(nd));
  }
