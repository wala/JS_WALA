    ...
    case 'AssignmentExpression':
      if(nd.operator === '=') {
        if(nd.left.type === 'Identifier') {
          ...
          throw new Error("cannot handle member expression lhs");
        }
      } else {
        ...
        if(nd.left.type === 'Identifier') {
          var tmp = context.genTmp();
          return rec(rhs, tmp).concat(rec(new ast.ExpressionStatement(new ast.AssignmentExpression('=', new ast.Identifier(nd.left.name), 
                                                                                                        new ast.BinaryExpression(op, new ast.Identifier(nd.left.name), new ast.Identifier(tmp))))));
    ...
    case 'UnaryExpression': 
      ...
      if(op === 'delete') {
        if(nd.argument.type === 'Identifier') {
          ...
          var target = context.getTarget();
          var base_tmp = context.genTmp(), index_tmp = context.genTmp();
          return rec(getBase(nd.argument), new ast.Identifier(base_tmp))
                .concat(rec(getIndex(nd.argument), new ast.Identifier(index_tmp)),
                        new ast.ExpressionStatement(new ast.AssignmentExpression('=', new ast.Identifier(target),
                                                                                      new ast.UnaryExpression('delete',
