...
define(function(require, exports) {
  ...
  function normalize(nd, context) {
    ...
    function normalizeFunction(nd, anonymize) {
      context.getTemp('target');
      context.pushFrame();
      var body = rec(nd.body);
      if(body.mayCompleteNormally())
      ...
      nd.localDecls().forEach(function(decl) {
        if(decl instanceof ast.VariableDeclarator || decl instanceof ast.FunctionDeclaration)
      ...
      context.popFrame();
      return splice("$target = $", new ast.FunctionExpression(anonymize ? null : nd.name, nd.params, new ast.BlockStatement(body)));
    }
    ...
    case ast.Script:
      ...
      var fundecls = new ast.List();
      nd.globalDecls().forEach(function(decl) {
        if(decl instanceof ast.FunctionDeclaration) {
          context.getTemp('tmp');
          fundecls.pushAll(normalizeFunction(decl));
          fundecls.pushAll(splice("$tmp = '" + decl.name + "';" +
          		                    "__global[$tmp] = $target;"));
        }
      });
      var temps = context.getTempDecls();
      return new ast.Script(nd.url, splice("(function(__global) { var $; $; $ })(this);", temps, fundecls, body));
    ...
    case ast.ReturnStatement:
      if(!nd.argument)
        ...
        return splice("$; return $tmp;", rec(nd.argument, "tmp"));
